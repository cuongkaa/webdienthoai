...
function themVaoGioHang(masp, tensp) {
    var user = getCurrentUser();
    if (!user) {
        addAlertBox('Bạn cần đăng nhập để thêm sản phẩm vào giỏ hàng!', '#e74c3c', '#fff', 3000);
        return;
    }
    // Tìm sản phẩm đã có trong giỏ
    var found = false;
    for (var sp of user.products) {
        if (sp.ma == masp) {
            sp.soluong += 1;
            found = true;
            break;
    ...
    if (!found) {
        user.products.push({ ma: masp, soluong: 1, date: new Date().getTime() });
    }
    setCurrentUser(user); // Lưu lại user vào LocalStorage
    updateListUser(user); // Thêm dòng này để cập nhật lại danh sách người dùng
    animateCartNumber(); // Cập nhật số lượng giỏ hàng trên giao diện
    capNhat_ThongTin_CurrentUser(); // Đảm bảo cập nhật số trên giao diện
    addAlertBox('Đã thêm ' + tensp + ' vào giỏ hàng!', '#27ae60', '#fff', 2000);
}
// ============================== TÀI KHOẢN ============================
